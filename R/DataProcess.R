...
dataProcess  <-  function(raw,
    ...
    processout <-
        as.matrix(read.table("sessionInfo.txt", header = TRUE, sep = "\t"))
    write.table(processout, file = finalfile, row.names = FALSE)
    
    processout <-
        rbind(processout, as.matrix(c(
    ...
    if (all(requiredInputUpper %in% providedInputUpper)) {
        processout <-
            rbind(processout, c("The required input : provided - okay"))
        write.table(processout, file = finalfile, row.names = FALSE)
        ...
        colnames(raw)[which(providedInputUpper == "PEPTIDEMODIFIEDSEQUENCE")]  <-
            "PeptideSequence"
    } else {
        ...
        write.table(processout, file = finalfile, row.names = FALSE)
        stop(
            "Please check the required input. The required input needs (ProteinName, PeptideSequence (or PeptideModifiedSequence), PrecursorCharge, FragmentIon, ProductCharge, IsotopeLabelType, Condition, BioReplicate, Run, Intensity)"
        )
    }
    ...
    if (logTrans != 2 & logTrans != 10) {
        processout <-
            rbind(processout,
                  c(
                      "ERROR : Logarithm transformation : log2 or log10 only - stop"
                  ))
        write.table(processout, file = finalfile, row.names = FALSE)
    ...
    if (!(fillIncompleteRows == TRUE |
          fillIncompleteRows == FALSE) | !is.logical(fillIncompleteRows)) {
        processout <-
            rbind(processout, c(
                paste(
                    "The required input - fillIncompleteRows : 'fillIncompleteRows' value is wrong. It should be either TRUE or FALSE. - stop"
                )
            ))
        write.table(processout, file = finalfile, row.names = FALSE)
    ...
    if (sum(summaryMethod == c("linear", "TMP")) == 0) {
        processout <-
            rbind(
                processout,
                c(
                    "The required input - summaryMethod : 'summaryMethod' value is wrong. It should be one of 'TMP' or 'linear'. - stop"
                )
            )
        write.table(processout, file = finalfile, row.names = FALSE)
        ...
        processout <-
            rbind(processout, c(paste(
                "summaryMethod : ", as.character(summaryMethod), sep = ""
            )))
        write.table(processout, file = finalfile, row.names = FALSE)
    }
    
    ## check input for cutoffCensored
    if (sum(cutoffCensored == c("minFeature", "minRun", "minFeatureNRun")) ==
        0) {
        processout <-
            rbind(
                processout,
                c(
                    "The required input - cutoffCensored : 'cutoffCensored' value is wrong. It should be one of 'minFeature','minRun','minFeatureNRun'. - stop"
                )
            )
        write.table(processout, file = finalfile, row.names = FALSE)
        
        stop(
            "'cutoffCensored' value is wrong. It should be one of 'minFeature','minRun','minFeatureNRun'."
        )
    } else {
        processout <-
            rbind(processout, c(paste(
                "cutoffCensored : ",
                as.character(cutoffCensored),
                sep = ""
            )))
        write.table(processout, file = finalfile, row.names = FALSE)
    }
    ...
    if (sum(censoredInt == c("0", "NA")) == 0 &
        !is.null(censoredInt)) {
        processout <-
            rbind(
                processout,
                c(
                    "The required input - censoredInt : 'censoredInt' value is wrong. It should be one of '0','NA', NULL. - stop"
                )
            )
        write.table(processout, file = finalfile, row.names = FALSE)
        ...
        processout <-
            rbind(processout, c(paste(
                "censoredInt : ", as.character(censoredInt), sep = ""
            )))
        write.table(processout, file = finalfile, row.names = FALSE)
    }
    ...
    if (summaryMethod == 'TMP' & MBimpute & is.null(censoredInt)) {
        processout <-
            rbind(
                processout,
                c(
                    "The rcombination of equired input - censoredInt and MBimpute : 'censoredInt=NULL' has no censored missing values. Imputation will not be performed.- stop"
                )
            )
        write.table(processout, file = finalfile, row.names = FALSE)
        
        stop(
            "'censoredInt=NULL' means that dataset has no censored missing value and MSstats will not impute. But, 'MBimpute=TRUE' is selected. Please replace by 'MBimpute=FALSE' or censoredInt='NA' or '0'"
        )
    ...
    if (!(
        ...
        normalization == "FALSE" |
        normalization == "EQUALIZEMEDIANS" |
        normalization == "QUANTILE" | normalization == "GLOBALSTANDARDS"
    )) {
        processout <-
            rbind(processout, c(
                paste(
                    "The required input - normalization : 'normalization' value is wrong. - stop"
                )
            ))
        write.table(processout, file = finalfile, row.names = FALSE)
        
        stop(
            "'normalization' must be one of \"None\", \"FALSE\", \"equalizeMedians\", \"quantile\", or \"globalStandards\". Please assign 'normalization' again."
        )
    }
    ...
    if (!is.element("NONE", normalization) &
        ...
        processout <-
            rbind(
                processout,
                c(
                    "ERROR : For normalization with global standards, the names of global standards are needed. Please add 'nameStandards' input."
                )
            )
        write.table(processout, file = finalfile, row.names = FALSE)
        
        stop (
            "For normalization with global standards, the names of global standards are needed. Please add 'nameStandards' input."
        )
    }
    ...
    if (is.factor(raw$Intensity) | is.character(raw$Intensity)) {
        suppressWarnings(raw$Intensity <-
                             as.numeric(as.character(raw$Intensity)))
    }
    ...
    if (any(is.na(raw$Run))) {
        processout <-
            rbind(
                processout,
                c(
                    "ERROR : There is missing information in 'Run' column. Please check 'Run' column."
                )
            )
        write.table(processout, file = finalfile, row.names = FALSE)
    ...
    if (any(is.na(raw$BioReplicate))) {
        processout <-
            rbind(
                processout,
                c(
                    "ERROR : There is missing information in 'BioReplicate' column. Please check 'BioReplicate' column."
                )
            )
        write.table(processout, file = finalfile, row.names = FALSE)
        
        stop (
            "There is missing information in 'BioReplicate' column. Please check 'BioReplicate' column."
        )
    }
    ...
    if (any(is.na(raw$Condition))) {
        processout <-
            rbind(
                processout,
                c(
                    "ERROR : There is missing information in 'Condition' column. Please check 'Condition' column."
                )
            )
        write.table(processout, file = finalfile, row.names = FALSE)
        
        stop (
            "There is missing information in 'Condition' column. Please check 'Condition' column."
        )
    }
    ...
    tempPeptide <-
        unique(raw[, c("PEPTIDESEQUENCE", "PRECURSORCHARGE")])
    tempPeptide$PEPTIDE <-
        paste(tempPeptide$PEPTIDESEQUENCE,
              tempPeptide$PRECURSORCHARGE,
              sep = "_")
    ...
    raw.temp <- data.frame(
        raw.temp,
        PEPTIDE = paste(raw.temp$PEPTIDESEQUENCE, raw.temp$PRECURSORCHARGE, sep =
                            "_"),
        TRANSITION = paste(raw.temp$FRAGMENTION, raw.temp$PRODUCTCHARGE, sep =
                               "_")
    )
    ...
    if (length(unique(raw.temp$ISOTOPELABELTYPE)) > 2) {
        processout <-
            rbind(
                processout,
                c(
                    "ERROR : There are more than two levels of labeling. So far, only label-free or reference-labeled experiment are supported. - stop"
                )
            )
        write.table(processout, file = finalfile, row.names = FALSE)
        
        stop(
            "Statistical tools in MSstats are only proper for label-free or with reference peptide experiments."
        )
    }
    ...
    work <- data.frame(
        PROTEIN = raw.temp$Protein,
        PEPTIDE = raw.temp$Peptide,
        TRANSITION = raw.temp$Transition,
        FEATURE = paste(raw.temp$Peptide, raw.temp$Transition, sep =
                            "_"),
    ...
    work$SUBJECT_ORIGINAL <-
        factor(work$SUBJECT_ORIGINAL, levels = unique(work$SUBJECT_ORIGINAL))
    work$LABEL <- factor(work$LABEL, levels = levels(work$LABEL))
    
    work[work$LABEL == "L", "GROUP"] <-
        work[work$LABEL == "L", "GROUP_ORIGINAL"]
    work[work$LABEL == "L", "SUBJECT"] <-
        work[work$LABEL == "L", "SUBJECT_ORIGINAL"]
    
    work <-
        data.frame(work, SUBJECT_NESTED = paste(work$GROUP, work$SUBJECT, sep =
                                                    "."))
    ...
    processout <-
        rbind(processout,
              c("New input format : made new columns for analysis - okay"))
    write.table(processout, file = finalfile, row.names = FALSE)
    
    
    ## 2016. 08.29 : replace <1 with zero for log2(intensity)
    if (length(which(!is.na(work$INTENSITY) &
                     work$INTENSITY < 1)) > 0) {
        processout <-
            rbind(processout, c(
                paste(
        ...
        message(
            paste(
                "** There are ",
                length(which(
                    !is.na(work$INTENSITY) & work$INTENSITY < 1
                )),
    ...
    if (checkMultirun$is.risky) {
        ...
        if (any(is.element(colnames(work), 'FRACTION'))) {
            ...
            info <-
                unique(work[, c('GROUP_ORIGINAL', 'SUBJECT_ORIGINAL', 'RUN')])
            info$condition <-
                paste(info$GROUP_ORIGINAL, info$SUBJECT_ORIGINAL, sep = "_")
            
            tmp <- work[!is.na(work$ABUNDANCE),]
            
            ## get on one sample first
            info.sample1 <-
                info[info$condition == unique(info$condition)[1],]
            ...
            for (k in 1:length(unique(info.sample1$RUN))) {
                ...
                unique.feature <-
                    unique(tmp[tmp$RUN %in% info.sample1$RUN[k], 'FEATURE'])
                
                tmptmp <- tmp[which(tmp$FEATURE %in% unique.feature),]
                tmptmp$condition <-
                    paste(tmptmp$GROUP_ORIGINAL,
                          tmptmp$SUBJECT_ORIGINAL,
                          sep = "_")
                
                count.feature <-
                    reshape2::dcast(
                        RUN ~ GROUP_ORIGINAL + SUBJECT_ORIGINAL,
                        data = tmptmp,
                ...
                same.frac <-
                    apply(count.feature[, -which(colnames(count.feature) %in% c('RUN'))], 2, function(x)
                        count.feature[which.max(x), 'RUN'])
            ...
            if (!checkup) {
                processout <-
                    rbind(processout, c(
                        paste(
                            "Multiple fractions are existed : ",
                            length(unique(work$FRACTION)),
                            "fractions per MS replicate."
                        )
                    ))
                write.table(processout,
                            file = finalfile,
        ...
        if (length(unique(work$FRACTION)) > 1) {
            ...
            count.fraction <-
                apply(count.feature[, -which(colnames(count.feature) %in% c('FEATURE'))], 1, function(x)
                    sum(x > 0))
            keep.feature <-
                count.feature[count.fraction == 1, 'FEATURE']
            ...
            if (nrow(count.feature1) > 0) {
                count.fraction <-
                    apply(count.feature1[, -which(colnames(count.feature1) %in% c('FEATURE'))], 1, function(x)
                        sum(x == max(x)))
                count.feature2 <-
                    count.feature1[count.fraction == 1,] ## except maximum, otherone replace with na
                
                if (nrow(count.feature2) > 0) {
                    remove.fraction <- apply(count.feature2, 1,
                                             function(x)
                                                 paste(x[1], names(x[-1])[x[-1] != max(x[-1]) &
                                                                              x[-1] != 0], sep = "_"))
                    remove.fraction <- unlist(remove.fraction)
                    
                    work[work$tmp %in% remove.fraction, 'INTENSITY'] <-
                        NA
                    work[work$tmp %in% remove.fraction, 'ABUNDANCE'] <-
                        NA
                }
                ...
                if (nrow(count.feature3) > 0) {
                    ...
                    max.feature <-
                        apply(count.feature3[, -which(colnames(count.feature3) %in% c('FEATURE'))], 1, function(x)
                            max(x))
                    max.feature.1 <-
                        count.feature3[max.feature == 1, 'FEATURE']
                    
                    work <-
                        work[-which(work$FEATURE %in% max.feature.1),]
                    
                    count.feature3 <-
                        count.feature3[-which(count.feature3$FEATURE %in% max.feature.1),]
                    ...
                    if (nrow(count.feature3) > 0) {
                        ...
                        tmptmp <-
                            work[which(work$FEATURE %in% count.feature3$FEATURE),]
                        tmptmp <- tmptmp[!is.na(tmptmp$ABUNDANCE),]
                        
                        mean.frac.feature <-
                            aggregate(ABUNDANCE ~ FEATURE + tmp,
                                      data = tmptmp,
                                      FUN = mean)
                        mean.frac.feature$FEATURE <-
                            factor(mean.frac.feature$FEATURE)
                        
                        temp2 <-
                            split(mean.frac.feature,
                                  mean.frac.feature$FEATURE)
                        
                        temp3 <- lapply(temp2, function(x) {
                            x <- x[order(x$ABUNDANCE, decreasing = TRUE), ]
                            x <-
                                x$tmp[2:nrow(x)] # get the fractionation which should be removed.
                        })
                        
                        remove.fraction <-
                            unlist(temp3, use.names = FALSE)
    ...
    processout <-
        rbind(processout, c(paste(
            "fillIncompleteRows = ", fillIncompleteRows, sep = ""
        )))
    write.table(processout, file = finalfile, row.names = FALSE)
    ...
    if (!checkMultirun$out | length(unique(work$FRACTION)) == 1) {
        ...
        if (nlevels(work$LABEL) == 1) {
            ...
            structure = tapply (work$ABUNDANCE, list (work$FEATURE, work$RUN) , function (x)
                length (x))
            ...
            if (flagmissing) {
                ...
                for (j in 1:length(runID)) {
                    ...
                    nameID <-
                        unique(work[work$RUN == runID[j], c(
                            "SUBJECT_ORIGINAL",
                            "GROUP_ORIGINAL",
                            "GROUP",
                            "SUBJECT",
                    ...
                    if (fillIncompleteRows) {
                        tempTogetfeature <-
                            work[which(work$FEATURE %in% names(finalfeatureID)),]
                        
                        ## get PROTEIN and FEATURE infomation
                        tempfeatureID <-
                            unique(tempTogetfeature[, c("PROTEIN",
                                                        "PEPTIDE",
                                                        "TRANSITION",
                                                        "FEATURE")])
                ...
                if (fillIncompleteRows) {
                    ...
                    processout <-
                        rbind(
                            processout,
                            "Incomplete rows for missing peaks are added with intensity values=NA. - done, Okay"
                        )
                    write.table(processout,
                                file = finalfile,
                                row.names = FALSE)
                    ...
                    processout <-
                        rbind(
                            processout,
                            "Please check whether features in the list are generated from spectral processing tool. Or the option, fillIncompleteRows=TRUE, will add incomplete rows for missing peaks with intensity=NA."
                        )
                    write.table(processout,
                                file = finalfile,
                                row.names = FALSE)
                    ...
                    stop(
                        "Please check whether features in the list are generated from spectral processing tool or not. Or the option, fillIncompleteRows=TRUE, will add incomplete rows for missing peaks with intensity=NA."
                    )
            ...
            if (flagduplicate) {
                ...
                for (j in 1:length(runID)) {
                    ...
                    featureID <- structure[, colnames(structure) == runID[j]]
                    finalfeatureID <-
                        featureID[!is.na(featureID) & featureID > 1]
            ...
            ## get feature by Run count of data
            structure.l = tapply(work.l$ABUNDANCE, list(work.l$FEATURE, work.l$RUN), function (x)
                length (x))
            structure.h = tapply(work.h$ABUNDANCE, list(work.h$FEATURE, work.h$RUN), function (x)
                length (x))
            ...
            featurestructure.h <-
                apply(structure.h, 1, function (x)
                    sum(is.na(x)))
            
            ## get feature ID of reference which are completely missing across run
            featureID.h <-
                names(featurestructure.h[featurestructure.h == ncol(structure.h)])
            
            if (length(featureID.h) > 0) {
                ## print message
                message(
                    paste(
                        "CAUTION : some REFERENCE features have missing intensities in all the runs. The completely missing REFERENCE features are ",
                ...
                if (fillIncompleteRows) {
                    ...
                    tempTogetfeature <-
                        work[which(work$FEATURE %in% featureID.h),]
                    tempfeatureID <-
                        unique(tempTogetfeature[, c("PROTEIN",
                                                    "PEPTIDE",
                                                    "TRANSITION",
            ...
            featurestructure.l <-
                apply(structure.l, 1, function (x)
                    sum(is.na(x)))
            
            ## get feature ID of reference which are completely missing across run
            featureID.l <-
                names(featurestructure.l[featurestructure.l == ncol(structure.l)])
            
            if (length(featureID.l) > 0) {
                ## print message
                message(
                    paste(
                        "CAUTION : some ENDOGENOUS features have missing intensities in all the runs. The completely missing ENDOGENOUS features are ",
                ...
                if (fillIncompleteRows) {
                    ...
                    ## here use whole work dataset
                    tempTogetfeature <-
                        work[which(work$FEATURE %in% featureID.l),]
                    tempfeatureID <-
                        unique(tempTogetfeature[, c("PROTEIN",
                                                    "PEPTIDE",
                                                    "TRANSITION",
            ...
            if (flagmissing.l | flagmissing.h) {
                ...
                if (flagmissing.l) {
                    if (length(missing.l) > 1) {
                        runstructure <-
                            apply (structure.l[which(rownames(structure.l) %in% missing.l),], 2, function (x)
                                sum (is.na (x))) > 0
                    } else if (length(missing.l) == 1) {
                        runstructure <-
                            is.na (structure.l[which(rownames(structure.l) %in% missing.l),]) > 0
                    }
                    ...
                    for (j in 1:length(runID)) {
                        ## get subject, group information for this run
                        nameID <-
                            unique(work.l[work.l$RUN == runID[j], c(
                                "SUBJECT_ORIGINAL",
                                "GROUP_ORIGINAL",
                                "GROUP",
                                "SUBJECT",
                        ...
                        if (length(missing.l) > 1) {
                            featureID <-
                                structure.l[which(rownames(structure.l) %in% missing.l), colnames(structure.l) == runID[j]]
                            
                            ## get feature ID which has no measuremnt.
                            finalfeatureID <-
                                names(featureID[is.na(featureID)])
                        ...
                        if (fillIncompleteRows) {
                            tempTogetfeature <-
                                work.l[which(work.l$FEATURE %in% finalfeatureID),]
                            ...
                            ## merge with tempary space, missingwork
                            missingwork.l <-
                                rbind(missingwork.l, tempmissingwork)
                        } # end fillIncompleteRows options
                    } # end loop for run ID
                } # end for endogenous
                ...
                if (flagmissing.h) {
                    ...
                    if (length(missing.h) > 1) {
                        runstructure <-
                            apply (structure.h[which(rownames(structure.h) %in% missing.h),], 2,
                                   function (x)
                                       sum (is.na (x))) > 0
                    } else if (length(missing.h) == 1) {
                        runstructure <-
                            is.na (structure.h[which(rownames(structure.h) %in% missing.h),]) > 0
                    }
                    ...
                    for (j in 1:length(runID)) {
                        ## get subject, group information for this run
                        nameID <-
                            unique(work.h[work.h$RUN == runID[j], c(
                                "SUBJECT_ORIGINAL",
                                "GROUP_ORIGINAL",
                                "GROUP",
                                "SUBJECT",
                        ...
                        if (length(missing.h) > 1) {
                            featureID <-
                                structure.h[which(rownames(structure.h) %in% missing.h), colnames(structure.h) == runID[j]]
                        ...
                        if (fillIncompleteRows) {
                            tempTogetfeature <-
                                work.h[which(work.h$FEATURE %in% finalfeatureID),]
                            ...
                            ## merge with tempary space, missingwork
                            missingwork.h <-
                                rbind(missingwork.h, tempmissingwork)
            ...
            if (flagduplicate.h) {
                ## first, which run has duplicates
                runstructure <-
                    apply (structure.h, 2, function (x)
                        sum (x[!is.na(x)] > 1) > 0)
                ...
                ## then for each run, which features have duplicates,
                for (j in 1:length(runID)) {
                    nameID <- unique(work[work$RUN == runID[j], c(
                        "SUBJECT_ORIGINAL",
                        "GROUP_ORIGINAL",
                        "GROUP",
                        "SUBJECT",
                    ...
                    featureID <-
                        structure.h[, colnames(structure.h) == runID[j]]
                    finalfeatureID <-
                        featureID[!is.na(featureID) & featureID > 1]
            ...
            if (flagduplicate.l) {
                ## first, which run has duplicates
                runstructure <-
                    apply (structure.l, 2, function (x)
                        sum (x[!is.na(x)] > 1) > 0)
                ...
                for (j in 1:length(runID)) {
                    ...
                    featureID <-
                        structure.l[, colnames(structure.l) == runID[j]]
                    finalfeatureID <-
                        featureID[!is.na(featureID) & featureID > 1]
                ...
                processout <-
                    rbind(processout,
                          "ERROR : Please remove duplicate rows in the list above. ")
                write.table(processout,
                            file = finalfile,
                            row.names = FALSE)
            ...
            ## no missing and no duplicates
            if (!flagmissing.h &
                !flagmissing.l & !flagduplicate.h & !flagduplicate.l) {
                processout <-
                    rbind(
                        processout,
                        c(
        ...
        for (k in 1:length(unique(work$FRACTION))) {
            ...
            structure = tapply (worktemp$ABUNDANCE, list (worktemp$FEATURE, worktemp$RUN) , function (x)
                length (x))
            
            ## structure value should be 2 for labeled, 1 for label-free, if not there are missingness
            if (nlevels(worktemp$LABEL) == 2) {
                ## label-based
                flag = sum(is.na(structure)) > 0 |
                    sum(structure[!is.na(structure)] < 2) > 0
            } else {
                ## label-free
                flag = sum(is.na(structure)) > 0
            }
            ...
            if (nlevels(worktemp$LABEL) == 2) {
                ...
                structure.h = tapply (worktemp.h$ABUNDANCE, list (worktemp.h$FEATURE, worktemp.h$RUN) , function (x)
                    length (x))
                structure.l = tapply (worktemp.l$ABUNDANCE, list (worktemp.l$FEATURE, worktemp.l$RUN) , function (x)
                    length (x))
                
                flagduplicate <-
                    sum(structure.h[!is.na(structure.h)] > 1) > 0 |
        ...
        if (sum(allflagmissing) != 0) {
            processout <-
                rbind(
                    processout,
                    c(
                        "CAUTION: the input dataset has incomplete rows. Missing feature intensities should be present in the dataset, and their intensities should be indicated with 'NA'. The incomplete rows are listed below."
                    )
                )
            write.table(processout,
                        file = finalfile,
            ...
            for (k in 1:length(unique(work$FRACTION))) {
                ...
                if (allflagmissing[k]) {
                    ...
                    if (nlevels(worktemp$LABEL) == 1) {
                        ...
                        for (j in 1:length(runID)) {
                            nameID <-
                                unique(worktemp[worktemp$RUN == runID[j], c(
                                    "SUBJECT_ORIGINAL",
                                    "GROUP_ORIGINAL",
                                    "GROUP",
                                    "SUBJECT",
                            ...
                            ## get feature ID
                            featureID <-
                                structure[, colnames(structure) == runID[j]]
                        ...
                        ## get feature by Run count of data
                        structure.l <-
                            tapply (work.l$ABUNDANCE, list(work.l$FEATURE, work.l$RUN), function (x)
                                length (x))
                        structure.h <-
                            tapply (work.h$ABUNDANCE, list(work.h$FEATURE, work.h$RUN), function (x)
                                length (x))
                        ...
                        if (length(featureID.h) > 0) {
                            ...
                            if (fillIncompleteRows) {
                                ...
                                tempTogetfeature <-
                                    worktemp[which(worktemp$FEATURE %in% featureID.h),]
                                tempfeatureID <-
                                    unique(tempTogetfeature[, c("PROTEIN",
                                                                "PEPTIDE",
                                                                "TRANSITION",
                        ...
                        if (length(featureID.l) > 0) {
                            ...
                            processout <-
                                rbind(processout, c(
                                    paste(
                                        "CAUTION : some ENDOGENOUS features have missing intensities in all the runs. The completely missing ENCOGENOUS features are ",
                                        paste(featureID.l, collapse = ", "),
                                        ". Please check whether features in the list are correctly generated from spectral processing tool. \n",
                                        sep = ""
                                    )
                            ...
                            if (fillIncompleteRows) {
                                ...
                                ## here use whole worktemp dataset
                                tempTogetfeature <-
                                    worktemp[which(worktemp$FEATURE %in% featureID.l),]
                                tempfeatureID <-
                                    unique(tempTogetfeature[, c("PROTEIN",
                                                                "PEPTIDE",
                                                                "TRANSITION",
                        ...
                        if (flagmissing.l | flagmissing.h) {
                            ...
                            if (flagmissing.l) {
                                ## first, which run has missing
                                runstructure <-
                                    apply (structure.l[-which(rownames(structure.l) %in% featureID.l), ], 2, function (x)
                                        sum (is.na (x))) > 0
                                ...
                                for (j in 1:length(runID)) {
                                    ...
                                    ## get feature ID
                                    featureID <-
                                        structure.l[-which(rownames(structure.l) %in% featureID.l), colnames(structure.l) ==
                                                        runID[j]]
                                    ...
                                    if (fillIncompleteRows) {
                                        tempTogetfeature <-
                                            work.l[which(
                                                work.l$FEATURE %in% names(finalfeatureID)
                                            ),]
                                        
                                        ## get PROTEIN and FEATURE infomation
                                        tempfeatureID <-
                                            unique(tempTogetfeature[, c(
                                                "PROTEIN",
                                                "PEPTIDE",
                                                "TRANSITION",
                                                "FEATURE"
                                        ...
                                        tempmissingwork <-
                                            data.frame(
                                                ...
                                                LABEL = "L",
                                                GROUP_ORIGINAL =
                                                    nameID$GROUP_ORIGINAL,
                                                SUBJECT_ORIGINAL =
                                                    nameID$SUBJECT_ORIGINAL,
                                                RUN = nameID$RUN,
                                                GROUP = nameID$GROUP,
                                                SUBJECT = nameID$SUBJECT,
                                                SUBJECT_NESTED =
                                                    nameID$SUBJECT_NESTED,
                                                INTENSITY = NA,
                                                ABUNDANCE = NA,
                                                FRACTION = nameID$FRACTION
                                            )
                            ...
                            if (flagmissing.h) {
                                ## first, which run has missing
                                runstructure <-
                                    apply (structure.h[-which(rownames(structure.h) %in% featureID.h), ], 2, function (x)
                                        sum (is.na (x))) > 0
                                ...
                                for (j in 1:length(runID)) {
                                    ...
                                    ## get feature ID
                                    featureID <-
                                        structure.h[-which(rownames(structure.h) %in% featureID.h), colnames(structure.h) ==
                                                        runID[j]]
                                    ...
                                    if (fillIncompleteRows) {
                                        tempTogetfeature <-
                                            work.h[which(
                                                work.h$FEATURE %in% names(finalfeatureID)
                                            ),]
            ...
            if (fillIncompleteRows) {
                ...
                write.table(processout,
                       ...
                       !is.null(missingwork.l) | !is.null(missingwork)) {
                ## save in process file.
                processout <-
                    rbind(
                        processout,
    ...
    work$SUBJECT_NESTED <-
        factor(work$SUBJECT_NESTED, levels = unique(work$SUBJECT_NESTED))
    ...
    if (is.element("NONE", normalization) |
        ...
        processout <-
            rbind(processout, c("Normalization : no normalization - okay"))
        write.table(processout, file = finalfile, row.names = FALSE)
    }
    ...
    if (!is.element("NONE", normalization) &
        ...
        if (nlevels(work$LABEL) == 1) {
            ...
            median.run.method  <-
                aggregate(ABUNDANCE ~ RUN + FRACTION,
                          data = work,
                          median,
                          na.rm = TRUE)
            median.method  <-
                tapply(
                    median.run.method$ABUNDANCE,
                    median.run.method$FRACTION,
                    median,
                    na.rm = TRUE
                )
            ...
            for (j in 1:length(nmethod)) {
                ...
                for (i in 1:length(namerun)) {
                    ...
                    work[namerun.idx, "ABUNDANCE"] <-
                        work[namerun.idx, "ABUNDANCE"] - median.run.method[median.run.method$RUN == namerun[i], "ABUNDANCE"] + median.method[j]
                }
            }
        }
    ...
    if (!is.element("NONE", normalization) &
        ...
        if (nlevels(work$LABEL) == 1) {
            ...
            work[!is.na(work$ABUNDANCE) &
                     work$ABUNDANCE == 0, 'ABUNDANCE'] <- 1
            
            for (j in 1:length(nmethod)) {
                namerun <- unique(work[work$FRACTION == nmethod[j], "RUN"])
                
                worktemp <-
                    work[which(work$RUN %in% namerun & !is.na(work$INTENSITY)), ]
                worktemp$RUN <- factor(worktemp$RUN)
                worktemp$FEATURE <- factor(worktemp$FEATURE)
                
                quantiletemp <-
                    as.matrix(xtabs(ABUNDANCE ~ FEATURE + RUN, data = worktemp))
                ...
                quantilelong <-
                    melt(quantiledone, id = rownames(quantiledone))
                colnames(quantilelong) <-
                    c("FEATURE", "RUN", "ABUNDANCE_quantile")
                rm(quantiledone)
        ...
        if (nlevels(work$LABEL) == 2) {
            ...
            for (j in 1:length(nmethod)) {
                ...
                worktemp <-
                    work[which(work$RUN %in% namerun & work$LABEL == "H"), ]
                worktemp$RUN <- factor(worktemp$RUN)
                worktemp$FEATURE <- factor(worktemp$FEATURE)
                ...
                worktemp.l <-
                    work[which(work$RUN %in% namerun & work$LABEL == "L"), ]
                worktemp.l$RUN <- factor(worktemp.l$RUN)
                worktemp.l$FEATURE <- factor(worktemp.l$FEATURE)
                
                quantiletemp.l <-
                    as.matrix(xtabs(ABUNDANCE ~ FEATURE + RUN, data = worktemp.l))
                rm(worktemp.l)
                ...
                quantiledone.l <-
                    quantiletemp.l - (quantiletemp - quantiledone)
                
                ## get quantiled to long format for apply difference endogenous
                quantilelong.l <-
                    melt(quantiledone.l, id = rownames(quantiledone.l))
                colnames(quantilelong.l) <-
                    c("FEATURE", "RUN", "ABUNDANCE_quantile")
                ...
                quantileall <-
                    rbind(quantileall, quantilelong.h, quantilelong.l)
                
            }
            
            ## merge with original data
            work <-
                merge(work, quantileall, by = c("FEATURE", "RUN", "LABEL"))
        ...
        processout <-
            rbind(processout,
                  c("Normalization : Quantile normalization - okay"))
        write.table(processout, file = finalfile, row.names = FALSE)
    }
    ...
    if (!is.element("NONE", normalization) &
        ...
        for (i in 1:length(nameStandards)) {
            ...
            namePeptide <-
                tempPeptide[tempPeptide$PEPTIDESEQUENCE == nameStandards[i], "PEPTIDE"]
            ...
            if (length(namePeptide) != 0) {
                ...
                ## if Proteins
                nameProtein <-
                    allProtein[allProtein == nameStandards[i]] # if we use 'grep', can' find the proteins name with some symbol, such as 'sp|P30153|2AAA_HUMAN'
                    ...
                    processout <-
                        rbind(processout, c(
                            paste(
                                ...
                                nameStandards[i] ,
                                ", is not in dataset. Please check whether 'nameStandards' input is correct or not."
                            )
                        ))
                    write.table(processout,
                                file = finalfile,
            ...
            tempStandard <-
                tempStandard[!is.na(tempStandard$ABUNDANCE), ]
            meanStandard <-
                tapply(tempStandard$ABUNDANCE, tempStandard$RUN, function(x)
                    mean(x, na.rm = TRUE))
            
            meanStandard <-
                data.frame(RUN = names(meanStandard), meanStandard)
        ...
        allmean <- apply(combine, 1, function(x)
            mean(x, na.rm = TRUE))
        ...
        median.all <-
            tapply(allmeantemp$allmean, allmeantemp$FRACTION, function(x)
                median(x, na.rm = TRUE))
        ...
        for (j in 1:length(nmethod)) {
            ...
            for (i in 1:length(namerun)) {
                ...
                if (!is.na(allmean[names(allmean) == namerun[i]]))
                    work[work$RUN == namerun[i] &
                             work$LABEL == "L", "ABUNDANCE"] <-
                        work[work$RUN == namerun[i] &
                                 work$LABEL == "L", "ABUNDANCE"] - allmean[names(allmean) == namerun[i]] +
                        median.all[j]
            }
        } # end loop method
        ...
        processout <-
            rbind(
                processout,
                c(
                    "Normalization : normalization with global standards protein - okay"
                )
            )
        write.table(processout, file = finalfile, row.names = FALSE)
    ...
    if (length(unique(work$FRACTION)) > 1) {
        ...
        check.multiple.run.feature <-
            apply(check.multiple.run.TF, 1, sum)
        
        ## each feature should be measured only in one method
        overlap.feature <-
            names(check.multiple.run.feature[check.multiple.run.feature > 1])
        
        if (length(overlap.feature) > 0) {
            message(
                paste(
                    "** Please check the listed featurues (",
            ...
            processout <-
                rbind(processout, c(
                    paste(
                        "** Please check the listed featurues (",
                        paste(overlap.feature, collapse = ", "),
                        ") Those features are measured across all fractionations. Please keep only one intensity of listed features among fractionations from one sample.",
                        sep = ""
                    )
                ))
            write.table(processout,
            ...
            stop(
                "Please keep only one intensity of listed features among fractinations from one sample. \n"
            )
        }
        ...
        if (any(is.element(colnames(work), 'TECHREPLICATE'))) {
            ...
            if (class(run.match) == "try-error") {
                ...
                run.match$GROUP_ORIGINAL <-
                    as.character(run.match$GROUP_ORIGINAL)
                run.match$SUBJECT_ORIGINAL <-
                    as.character(run.match$SUBJECT_ORIGINAL)
                
                for (k in 1:nrow(run.match)) {
                    work[which(work$originalRUN %in% run.match[k, 4:ncol(run.match)]), 'newRun'] <-
                        paste(paste(run.match[k, 1:4], collapse = "_"),
                              'merged',
                              sep = "_")
                }
                ...
                na.count.long <-
                    melt(na.count, id.vars = c('FEATURE'))
                na.count.long <-
                    na.count.long[na.count.long$value == length(unique(work$newRun)),]
                na.count.long$tmp <-
                    paste(na.count.long$FEATURE,
                          na.count.long$variable,
                          sep = "_")
                
                work$tmp <-
                    paste(work$FEATURE, work$FRACTION, sep = "_")
                
                work <-
                    work[-which(work$tmp %in% na.count.long$tmp),]
                ##
                ...
                work <-
                    work[, -which(colnames(work) %in% c('tmp', 'newRun'))]
                
            }
        } else {
            ## Fraction, but no tech replicate
            ...
            if (class(run.match) == "try-error") {
                ...
                for (k in 1:nrow(run.match)) {
                    work[which(work$originalRUN %in% run.match[k, 3:ncol(run.match)]), 'newRun'] <-
                        paste(paste(run.match[k, 1:3], collapse = "_"),
                              'merged',
                              sep = "_")
                }
    ...
    if (betweenRunInterferenceScore) {
        ...
        temp <-
            tapply(l$ABUNDANCE, l[, c("RUN", "ProtPeptide")], function(x)
                mean(x, na.rm = TRUE))
        temp1 <-
            data.frame(
        ...
        temp4 <-
            tapply(rownames(temp3), temp3[, c("ProtFeature")], function(x)
                cor(temp3[x, "ABUNDANCE"], temp3[x, "meanPEPTIDE"]))
        
        names <-
            unique(temp2[, c("PROTEIN", "PEPTIDE", "FEATURE", "ProtFeature")])
        names <- names[with(names, order(ProtFeature)), ]
        BetweenRunInterferenceFile <-
            data.frame(names[, c("PROTEIN", "PEPTIDE", "FEATURE")], BetweenRunInterferenceScore =
                           temp4)
        
        BetweenRunInterferenceFile <-
            BetweenRunInterferenceFile[with(BetweenRunInterferenceFile,
                                            order(PROTEIN, PEPTIDE, FEATURE)), ]
        
        write.table(
            BetweenRunInterferenceFile,
            file = paste(address, "BetweenRunInterferenceFile.txt", sep = "")
        )
        ...
        processout <-
            rbind(
                processout,
                c(
                    "Between Run Interference Score is calculated and saved in .csv file - okay"
                )
            )
        write.table(processout, file = finalfile, row.names = FALSE)
        ...
        processout <-
            rbind(processout,
                  c("Between Run Interference Score is not calculated."))
        write.table(processout, file = finalfile, row.names = FALSE)
    }
    ...
    if (summaryMethod == "TMP" & MBimpute) {
        ...
        if (!is.null(maxQuantileforCensored)) {
            ...
            if (!label) {
                ...
                tmp <-
                    work[!is.na(work$INTENSITY) & work$INTENSITY > 1, 'ABUNDANCE']
                ## or
                ...
                log2int.prime.quant <-
                    quantile(
                        tmp,
                        prob = c(0.01, 0.25, 0.5, 0.75, maxQuantileforCensored),
                        na.rm = TRUE
                    )
                ...
                multiplier <-
                    (log2int.prime.quant[5] - log2int.prime.quant[4]) / iqr
                
                cutoff.lower <-
                    (log2int.prime.quant[2] - multiplier * iqr)
                
                work[!is.na(work$INTENSITY) &
                         work$ABUNDANCE < cutoff.lower, 'censored'] <-
                    TRUE
                ...
                if (cutoff.lower <= 0 &
                    !is.null(censoredInt) & censoredInt == "0") {
                    work[!is.na(work$INTENSITY) &
                             work$INTENSITY == 1, 'censored'] <- TRUE
                    work[!is.na(work$ABUNDANCE) &
                             work$ABUNDANCE <= 0, 'censored'] <- TRUE
                    
                    message(
                        paste(
                            '** Log2 intensities = 0 were considered as censored missing values.'
                        )
                    )
            ...
            if (label) {
                ...
                ## only consider intensity > 1
                tmp <-
                    work.tmp[!is.na(work.tmp$INTENSITY) &
                                 work.tmp$INTENSITY > 1, 'ABUNDANCE']
                ...
                message(
                    paste(
                        '** Log2 endogenous intensities under cutoff =',
                        format(cutoff.lower, digits = 5),
                        ' were considered as censored missing values.'
                    )
                )
                ...
                if (cutoff.lower <= 0 &
                    ...
                    work[work$LABEL == 'L' &
                             ...
                             work$INTENSITY == 1, 'censored'] <- TRUE
                    work[work$LABEL == 'L' &
                             !is.na(work$ABUNDANCE) &
                             work$ABUNDANCE <= 0, 'censored'] <- TRUE
                    
                    message(
                        paste(
                            '** Log2 endogenous intensities = 0 were considered as censored missing values.'
                        )
                    )
            ...
            ## will MBimpute, but not apply algorithm for cutoff
    ...
    if (featureSubset == "all") {
        ...
        processout <-
            rbind(processout,
                  c("** Use all features that the dataset origianally has."))
        write.table(processout, file = finalfile, row.names = FALSE)
    }
    
    if (featureSubset == "highQuality") {
        message(
            "** Selecting high quality features temporarily defaults to featureSubset = top3. Updates for this option will be available in the next release."
        )
    ...
    if (featureSubset == "top3") {
        ...
        processout <-
            rbind(
                processout,
                c(
                    "** Use top3 features that have highest average of log2(intensity) across runs."
                )
            )
        write.table(processout, file = finalfile, row.names = FALSE)
        ...
        temp1 <-
            aggregate(INTENSITY ~ PROTEIN + FEATURE, data = work, function(x)
                mean(x, na.rm = TRUE))
    ...
    if (featureSubset == "topN") {
        ...
        worktemp <-
            work[!is.na(work$ABUNDANCE) & work$ABUNDANCE != 0,]
        temp1 <-
            aggregate(INTENSITY ~ PROTEIN + FEATURE, data = worktemp, function(x)
                mean(x, na.rm = TRUE))
    ...
    if (nlevels(work$LABEL) == 1) {
        ...
        all.work <- work
        test <-
            tapply(is.na(work[, "ABUNDANCE"]), work[, c("GROUP_ORIGINAL", "FEATURE")], function(x)
                sum(x, na.rm = TRUE))
        numObs <-
            tapply(work[, "ABUNDANCE"], work[, c("GROUP_ORIGINAL", "FEATURE")], function(x)
                length(x))
        test1 <- test == numObs
    ...
    if (nlevels(work$LABEL) == 2) {
        ...
        for (i in 1:ncol(decision)) {
            for (j in 1:nrow(decision)) {
                ...
                if (count.light[j, i] == TRUE ||
                    count.heavy[j, i] == TRUE) {
                    decision[j, i] <- 1
                }
            }
        }
        ...
        for (i in 1:length(final.decision)) {
            ...
            if (final.decision[i] != 0) {
                work[work$FEATURE == names(final.decision[i]), "SuggestToFilter"] <-
                    1
            }
        }
    }
    ...
    processout <-
        rbind(processout, c(paste(
            rownames(summary.f)[1], " : ", summary.f[1], sep = ""
        )))
    processout <-
        rbind(processout, c(paste(
            rownames(summary.f)[2], " : ", summary.f[2], sep = ""
        )))
    processout <-
        rbind(processout, c(paste(
            rownames(summary.f)[3], " : ", summary.f[3], sep = ""
        )))
    ...
    if (nrow(temp2) > 0) {
        message(
            "\n",
            "** Protein (",
            paste(rownames(temp2), collapse = ", "),
            ") has only single transition : Consider excluding this protein from the dataset.",
            "\n"
        )
    }
    ...
    summary.s <- matrix(NA,
                        ncol = nlevels(work$GROUP_ORIGINAL),
                        nrow = 3)
    ...
    c.tech <-
        round(summary.s[1, ] / (summary.s[2, ] * length(unique(work$FRACTION))))
    ...
    message("\n Summary of Missingness :\n")
    message(
        "  # transitions are completely missing in one condition: ",
    ...
    message("\n  # run with 75% missing observations: ",
            sum(run.missing < 0.25),
            "\n")
    ...
    if (sum(final.decision != 0) != 0) {
        ...
        if (length(tmp.final) > 5) {
            processout <-
                rbind(processout,
                      "    -> ",
                      paste(names(tmp.final[1:5]), collapse = ", "),
                      " ...")
            ...
            processout <-
                rbind(processout,
                      "    -> ",
                      paste(names(tmp.final), collapse = ", "),
                      " ...")
        }
        rm(tmp.final)
    ...
    if (sum(run.missing < 0.25) != 0) {
        processout <-
            rbind(processout, "    -> ", paste("RUN", names(without[run.missing < 0.25]), sep =
                                                   " "))
    }
    write.table(processout, file = finalfile, row.names = FALSE)
    ...
    processout <-
        rbind(processout, c("Processing data for analysis is done. - okay"))
    write.table(processout, file = finalfile, row.names = FALSE)
    ...
    if (class(rqresult) == "try-error") {
        message("*** error : can't summarize per subplot with ",
                summaryMethod,
                ".")
        ...
        processout <-
            rbind(processout, c(
                paste(
                    "the summarization per subplot is done.- okay : ",
                    summaryMethod,
                    sep = ""
                )
            ))
    ...
    if (any(is.element(colnames(rqall), "RUN"))) {
        rqall <-
            rqall[order(rqall$Protein, as.numeric(as.character(rqall$RUN))), ]
        rownames(rqall) <- NULL
    }
...
########################################################
.runQuantification <- function(data,
                               summaryMethod,
                               equalFeatureVar,
                               cutoffCensored,
                               censoredInt,
    ...
    if (summaryMethod == "linear"  & is.null(censoredInt)) {
        ...
        for (i in 1:nlevels(data$PROTEIN)) {
            ...
            if (!label) {
                ...
                sub.result <-
                    data.frame(
                        Protein = rep(unique(sub$PROTEIN),
            ...
            TechReplicate <-
                .checkTechReplicate(sub) ## use for label-free model
            
            ##### fit the model
            #if (message.show) {
            message(
                paste(
                    "Getting the summarization per subplot for protein ",
                    unique(sub$PROTEIN),
                    "(",
                    i,
                    " of ",
            ...
            if (class(fit) == "try-error") {
                message("*** error : can't fit the model for ",
                ...
                for (j in 1:nlevels(sub$RUN)) {
                    ...
                    contrast <-
                        .make.contrast.run.quantification(fit, contrast.matrix, sub, labeled =
                                                              label)
                ...
                if (label) {
                    contrast <-
                        .make.contrast.run.quantification.reference(fit, contrast.matrix, sub)
        ...
        MS_results <-
            foreach(
                ...
                if (message.show) {
                    message(
                        paste(
                            "Getting the summarization by Tukey's median polish per subplot for protein ",
                            unique(sub$PROTEIN),
                            "(",
                            i,
                            " of ",
                ...
                if (nrow(sub) == (sum(is.na(sub$ABUNDANCE)) + sum(!is.na(sub$ABUNDANCE) &
                                                                  sub$ABUNDANCE == 0))) {
                    message(
                        paste(
                            "Can't summarize for ",
                ...
                if (MBimpute) {
                    if (!is.null(censoredInt)) {
                        ...
                        if (censoredInt == "0") {
                            subtemp <-
                                sub[sub$LABEL == "L" &
                                        !is.na(sub$ABUNDANCE) & sub$ABUNDANCE != 0,]
                    ...
                    if (nrow(sub) == 0) {
                        message(
                            paste(
                                ...
                                unique(sub$PROTEIN),
                                "(", i, " of ", length(unique(data$PROTEIN)),
                                ") because features have only one measurement",
                                "across MS runs."
                            )
                        )
                        # next()
                ...
                if (nrow(sub) == (sum(is.na(sub$ABUNDANCE)) + sum(!is.na(sub$ABUNDANCE) &
                    ...
                    message(
                        paste(
                            "After removing features which has only 1",
                            "measurement, Can't summarize for ",
                            unique(sub$PROTEIN),
                            "(", i, " of ",
                            length(unique(data$PROTEIN)),
                            ") because all measurements are NAs."
                ...
                if (length(norun) != 0 &
                    length(intersect(
                        norun, as.character(unique( sub$RUN ))))) {
                ...
                if (remove50missing) {
                    ...
                    if (!is.null(censoredInt)) {
                        ...
                        if (censoredInt == "0") {
                            subtemp <-
                                sub[sub$LABEL == "L" & !is.na(sub$INTENSITY) & sub$INTENSITY != 0, ]
                        }
                    }
                ...
                if (sum(sub$cen == 0) > 0) {
                    ...
                    if (cutoffCensored == "minFeature") {
                        if (censoredInt == "NA") {
                            cut <-
                                aggregate(ABUNDANCE ~ feature.label, data = sub, function(x)
                                    min(x, na.rm = TRUE))
                            ...
                            for (j in 1:length(unique(cut$feature.label))) {
                                sub[is.na(sub$ABUNDANCE) &
                                        sub$feature.label == cut$feature.label[j], "ABUNDANCE"] <-
                                    cut$ABUNDANCE[j]
                            }
                        }
                        ...
                        if (censoredInt == "0") {
                            ...
                            cut <-
                                aggregate(ABUNDANCE ~ feature.label,
                                          data = subtemptemp,
                                          FUN = min)
                            ## cutoff for each feature is little less than minimum abundance in a run.
                            cut$ABUNDANCE <- 0.99 * cut$ABUNDANCE
                    ...
                    if (cutoffCensored == "minRun") {
                        ...
                        if (censoredInt == "NA") {
                            cut <-
                                aggregate(ABUNDANCE ~ run.label, data = sub, function(x)
                                    min(x, na.rm = TRUE))
                            ...
                            if (length(unique(cut.fea$feature.label)) > 1) {
                                for (j in 1:length(unique(cut.fea$feature.label))) {
                                    for (k in 1:length(unique(cut.run$run.label))) {
                                        ...
                                        finalcut <- min(cut.fea$ABUNDANCE[j],
                                                        cut.run$ABUNDANCE[k])
                        ...
                        if (!label) {
                            ...
                            if (nrow(sub[sub$cen == 0,]) > 0) {
                                ...
                                if (length(unique(sub$FEATURE)) == 1) {
                                    fittest <-
                                        survival::survreg(
                                            survival::Surv(ABUNDANCE, cen, type = 'left') ~ RUN,
                                            data = sub,
                                            dist = 'gaussian'
                                        )
                                } else{
                                    if (countdf) {
                                        ...
                                        fittest <-
                                            survival::survreg(
                                                survival::Surv(ABUNDANCE, cen, type = 'left') ~ FEATURE + RUN,
                                                data = sub,
                                                dist = 'gaussian'
                                            )
                                    }
                            ...
                            if (nrow(sub.l[sub.l$cen == 0,]) > 0) {
                                ...
                                # the replace censored value with predicted value
                                sub.l[sub.l$cen == 0, "ABUNDANCE"] <-
                                    sub.l[sub.l$cen == 0, "pred"]
                ...
                if (nlevels(sub$FEATURE) > 1) {
                    ...
                    if (!label) {
                        ...
                        if (!is.null(censoredInt)) {
                            if (censoredInt == "NA") {
                                ...
                                subtempimpute <-
                                    subtempimpute[!is.na(subtempimpute$ABUNDANCE),]
                            }
                            ...
                            subtemp$RUN <-
                                factor(subtemp$RUN, levels = rownames(data_w))
                            numFea <- xtabs( ~ RUN, subtemp)
                            numFeaPercentage <-
                                1 - numFea / length(unique(subtemp$FEATURE))
                            numFeaTF <- numFeaPercentage >= 0.5
                            
                            subtempimpute$RUN <-
                                factor(subtempimpute$RUN, levels = rownames(data_w))
                        ...
                        for (k in 1:length(unique(h$RUN))) {
                            ## ABUNDANCE is normalized
                            reformresult.logical <-
                                reformresult$RUN == unique(h$RUN)[k]
                            reformresult.idx <- which(reformresult.logical)
                            reformresult[reformresult.idx, "ABUNDANCE"] <-
                                reformresult[reformresult.idx, "ABUNDANCE"] - reformresult[reformresult.logical &
                                                                                               reformresult$LABEL == "H", "ABUNDANCE"] + allmed
                        }
                        
                        reformresult <-
                            reformresult[reformresult$LABEL == "L",]
                        
                        subtemp <-
                            reformresult[!is.na(reformresult$ABUNDANCE),]
                    ...
                    if (label) {
                        ...
                        for (k in 1:length(unique(h$RUN))) {
                            ...
                            sub[subrun.idx, "ABUNDANCE"] <-
                                sub[subrun.idx, "ABUNDANCE"] - sub[subrun.logical &
                                                                       sub$LABEL == "H", "ABUNDANCE"] + allmed
                        }
        ...
        if (label) {
            ...
            for (i in 1:length(unique(data$PROTEIN))) {
                ...
                if (message.show) {
                    message(
                        paste(
                            "Getting the summarization for censored missing values per subplot for protein ",
                            unique(sub$PROTEIN),
                            "(",
                            i,
                            " of ",
                ...
                if (length(norun) != 0 &
                    length(intersect(norun, as.character(unique(
                        sub$RUN
                    ))))) {
                    # removed NA rows already, if there is no overlapped run, error
                    sub <- sub[-which(sub$RUN %in% norun), ]
                ...
                ## 1. censored
                if (censoredInt == "0") {
                    sub$cen <- ifelse(!is.na(sub$INTENSITY) & sub$INTENSITY == 0,
                                      0,
                                      1)
                }
                ...
                subtemp <- sub[!is.na(sub$ABUNDANCE), ]
                countdf <-
                    nrow(subtemp) < (length(unique(subtemp$FEATURE)) + length(unique(subtemp$RUN)) -
                                         1)
                ...
                if (length(unique(sub$FEATURE)) == 1) {
                    ...
                    fittest <-
                        survival::survreg(
                            survival::Surv(ABUNDANCE, cen, type = 'left') ~ RUN + ref,
                            data = sub,
                            dist = 'gaussian'
                        )
                } else{
                    if (countdf) {
                        ...
                        fittest <-
                            survival::survreg(
                                survival::Surv(ABUNDANCE, cen, type = 'left') ~ FEATURE + RUN + ref,
                                data = sub,
                                dist = 'gaussian'
                            )
                    }
                ...
                for (j in 1:nlevels(sub$RUN)) {
                    ...
                    contrast.matrix[j] <- 1
                    contrast <-
                        .make.contrast.run.quantification.Survival(fittest, contrast.matrix, sub, labeled =
                                                                       TRUE)
                    
                    sub.result[a, 3] <-
                        .estimableFixedQuantificationSurvival(cf, contrast)
                    a = a + 1
                }
            ...
            for (i in 1:length(unique(data$PROTEIN))) {
                ...
                if (nrow(sub) == 0) {
                    message(
                        paste(
                            "* All measurements are NAs or only one measurement per feature in ",
                            levels(data$PROTEIN)[i],
                ...
                if (cutoffCensored == "minFeature") {
                    if (censoredInt == "NA") {
                        cut <-
                            aggregate(ABUNDANCE ~ FEATURE, data = sub, function(x)
                                min(x, na.rm = TRUE))
                        ## cutoff for each Run is little less than minimum abundance in a run.
                        cut$ABUNDANCE <- 0.99 * cut$ABUNDANCE
                ...
                if (cutoffCensored == "minRun") {
                    if (censoredInt == "NA") {
                        cut <-
                            aggregate(ABUNDANCE ~ RUN, data = sub, function(x)
                                min(x, na.rm = TRUE))
                        ## cutoff for each Run is little less than minimum abundance in a run.
                        cut$ABUNDANCE <- 0.99 * cut$ABUNDANCE
...
.fit.quantification.run <-
    function(sub,
        ...
        if (!equalFeatureVar) {
            fit.full <- .iter.wls.fit.model(data = sub,
                                            fit = fit.full,
                                            nrepeats = 1)
        }
...
.countMultiRun <- function(data) {
    ...
    if (any(is.element(colnames(data), 'FRACTION'))) {
        ...
        info <-
            unique(data[, c('GROUP_ORIGINAL', 'SUBJECT_ORIGINAL', 'RUN')])
        info$condition <-
            paste(info$GROUP_ORIGINAL, info$SUBJECT_ORIGINAL, sep = "_")
        ...
        if (!is.multiplerun) {
            ...
            info.sample1 <-
                info[info$condition == unique(info$condition)[1],]
            tmp.sample1 <-
                tmp[tmp$GROUP_ORIGINAL == unique(info.sample1$GROUP_ORIGINAL) &
                        tmp$SUBJECT_ORIGINAL == unique(info.sample1$SUBJECT_ORIGINAL),]
            
            standardFeature <-
                unique(tmp.sample1[tmp.sample1$RUN == unique(tmp.sample1$RUN[1]),
                                   "FEATURE"])
            ...
            countdiff <- tapply (tmp.sample1$FEATURE,
                                 tmp.sample1$RUN,
                                 function (x)
                                     length(intersect(unique(x), standardFeature)))
            
            per.overlap.feature <-
                (countdiff)[-1] / max(unique(countdiff))
            
            ## first, technical replicate, no fraction :
            ## all runs should have more than 50% features should be the same.
            if (all(per.overlap.feature > 0.5)) {
                ## then there are technical replicates
                out <- FALSE
